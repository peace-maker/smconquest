...
// ConfigSection defined smconquest_models.sp
		...
		if(GetArrayCell(hClass, CLASS_DEFAULT) == 1)
		{
			...
			if(iClassTeam == 0)
			{
				...
				if(iDefault[0] != -1 || iDefault[1] != -1 || iDefault[2] != -1)
					SetFailState("Error parsing classes. There are duplicate \"default\" settings! Only set one global default or one default per team.");
				iDefault[0] = i;
			}
			else
			{
